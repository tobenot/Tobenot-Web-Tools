...
export function MonthGrid({ year, month, dayInfos, todayIso, selectedIso, getHolidayStatus, onSelect }: MonthGridProps) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-1 -mx-3 sm:mx-0">
            ...
            {week.map((cell, ci) => {
              ...
              return (
                <button
                  ...
                  title={holidayTheme.title ?? undefined}
                  className={
                    'relative bg-mech-panel border border-mech-edge rounded-[3px] flex flex-col items-stretch justify-center select-none py-1 px-1 sm:px-1.5 min-h-[52px] ' +
                    (cell.day
                      ? 'hover:border-mech-accent cursor-pointer ' +
                        (isSelected ? 'border-mech-accent ring-1 ring-mech-accent/30 ' : '') +
                        (holidayTheme.kind === 'off' ? 'bg-rose-50/60 border-rose-200 ' : '') +
                        (holidayTheme.kind === 'work' ? 'bg-amber-50/60 border-amber-200 ' : '') +
                        (isToday ? 'ring-2 ring-blue-300/70 ' : '')
                      : 'opacity-30 cursor-default')
                  }
                  aria-pressed={!!isSelected}
                >
                  <span className="flex items-center justify-between w-full h-4">
                    <span className={
                      'tabular-nums text-sm leading-none ' +
                      (holidayTheme.kind === 'off' ? 'font-semibold text-rose-600 ' : '') +
                      (holidayTheme.kind === 'work' ? 'font-semibold text-amber-700 ' : '') +
                    ...
                    {holidayTheme.badge && (
                      <span className={
                        'inline-flex shrink-0 items-center justify-center w-4 h-4 rounded-[2px] text-[9px] leading-none font-bold text-white ' +
                        (holidayTheme.kind === 'off' ? 'bg-rose-500' : 'bg-amber-500')
                      }>
                        {holidayTheme.badge}
                      </span>
                    )}
                  ...
                  {info && cell.day && (
                    <span className={`text-[10px] leading-tight truncate max-w-full text-left ${getCellLabelColor(info)}`}>
                      {getCellLabel(info)}
                    </span>
                  )}
                </button>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        {/* 图例 */}
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-mech-muted">
          <span className="flex items-center gap-1"><span className="inline-flex items-center justify-center w-4 h-4 rounded-[2px] bg-rose-500 text-[9px] leading-none font-bold text-white">休</span>放假</span>
          <span className="flex items-center gap-1"><span className="inline-flex items-center justify-center w-4 h-4 rounded-[2px] bg-amber-500 text-[9px] leading-none font-bold text-white">班</span>调休上班</span>
